...
test.describe('Поиск элементов по роли "button"', () => {
  ...
  test('Найти неактивную кнопку по роли и состоянию', async ({ page }) => {
    const disabledButton = page.getByRole('button', { name: 'Неактивная кнопка', disabled: true }); // TODO(student): замените на корректный локатор
...
test.describe('Поиск элементов форм по ролям', () => {
  ...
  test('Заполнить и отправить форму', async ({ page }) => {
    ...
    await expect(usernameInput).toBeVisible();
    await usernameInput.fill('Aleksey');

    const userpasswordInput = page.getByRole('textbox', { name: 'Пароль' });
    await expect(userpasswordInput).toBeVisible();
    await userpasswordInput.fill('Ui123456$%');

    const usercountryInput = page.getByRole('combobox', { name: 'Страна' });
    await expect(usercountryInput).toBeVisible();
    await usercountryInput.selectOption('Россия');

    const buttonsubmit = page.getByRole('button', { name: 'Отправить' });
