...
test.describe('Сложные случаи и динамический контент', () => {
  ...
  test('Найти элемент с title с пробелами', async ({ page }) => {
    const spacedTitle = page.getByTitle(/\s+Подсказка с пробелами+\s/); // TODO(student): замените на корректный локатор
  ...
  test('Найти изображение по title', async ({ page }) => {
    ...
    await expect(image).toHaveJSProperty('width', 225);
    await expect(image).toHaveJSProperty('height', 225);
  });
});
