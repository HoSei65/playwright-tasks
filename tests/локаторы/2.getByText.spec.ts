...
test.describe('Базовые тесты для getByText()', () => {
  ...
  test('Найти элемент по точному тексту', async ({ page }) => {
    const paragraph = page.getByText('Это обычный параграф текста для поиска'); // TODO(student): замените на корректный локатор
...
test.describe('Сложные случаи поиска по тексту', () => {
  ...
  test('Работа с динамическим контентом', async ({ page }) => {
    const dynamicText = page.getByText('Динамически загруженный текст'); // TODO(student): замените на корректный локатор
