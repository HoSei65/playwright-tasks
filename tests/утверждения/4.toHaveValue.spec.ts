...
test('3. Проверка сброса формы', async ({ page }) => {
  ...
  await expect(page.getByLabel('Имя пользователя')).toHaveValue('Петр');
  await expect(page.getByLabel('Электронная почта')).toHaveValue('test@test.ru');
  await expect(page.getByLabel('Страна')).toHaveValue('by');
